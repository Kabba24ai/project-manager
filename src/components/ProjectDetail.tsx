      ...
      try {
        ...
        if (response.data?.project) {
          ...
          const transformedProject: Project = {
            ...
            priority: backendProject.priority as any,
            createdAt: backendProject.created_at,
            dueDate: backendProject.due_date,
            tasksCount: backendProject.tasks_count,
            completedTasks: backendProject.completed_tasks,
            progressPercentage: backendProject.progress_percentage,
            team: backendProject.team?.map((member: any) => ({
              id: member.id,
              name: member.name,
              email: member.email,
          ...
          const transformedTaskLists: TaskList[] = backendProject.task_lists?.map((list: any) => ({
            id: list.id,
            name: list.name,
            ...
            tasks: list.tasks?.map((task: any) => ({
              ...
              assignedTo: {
                id: task.assigned_to?.id || 0,
                name: task.assigned_to?.name || 'Unassigned',
                email: task.assigned_to?.email || '',
                avatar: task.assigned_to?.avatar || task.assigned_to?.initials || 'UN',
                role: task.assigned_to?.role as any || 'developer'
              },
          ...
          <div className="flex items-center justify-between">
              ...
              <div>
                <div className="flex items-center space-x-3">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium border ${
                    projectData.priority === 'urgent' ? 'bg-red-100 text-red-800 border-red-200' :
                    projectData.priority === 'high' ? 'bg-orange-100 text-orange-800 border-orange-200' :
                    projectData.priority === 'medium' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                ...
                <div className="flex items-center space-x-6 mt-3 text-sm text-gray-500">
                  ...
                  <div className="flex items-center space-x-1">
                    <Calendar className="w-4 h-4" />
                    <span>Due: {projectData.dueDate ? formatDateLocal(projectData.dueDate) : 'No due date'}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <span>{projectData.completedTasks}/{projectData.tasksCount} tasks completed</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <span>{projectData.progressPercentage}% progress</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              ...
              <div className="flex -space-x-2">
                {projectData.team.slice(0, 4).map((member, index) => (
                  <div
                    key={member.id}
                    className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center border-2 border-white text-xs font-medium text-white"
                    title={member.name}
                  >
                    {member.avatar}
                  </div>
                ))}
                {projectData.team.length > 4 && (
